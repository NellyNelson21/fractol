...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/10 17:38:42 by nnelson           #+#    #+#             */
/*   Updated: 2022/03/12 18:53:04 by nnelson          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int	calculate_burningship_plenty(t_mlx *mlx)
{
	int		i;
	double	tempx;
		...
		&& (i < mlx->iteration_limit))
	{
		...
		mlx->f->z_y = fabs(2 * mlx->f->z_x * mlx->f->z_y) + mlx->f->const_y;
		mlx->f->z_x = tempx;
		i++;
	}
	return (i);
...
void	draw_burningship(t_mlx *mlx)
{
	...
	while (x < WIN_SIZE)
	{
		...
		while (y < WIN_SIZE)
		{
			...
			mlx->f->z_x = 0;
			mlx->f->z_y = 0;
			iterations = calculate_burningship_plenty(mlx);
