...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/10 16:53:37 by nnelson           #+#    #+#             */
/*   Updated: 2022/03/14 17:12:35 by nnelson          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int	calculate_julia_plenty(t_mlx *mlx)
{
	int		i;
	double	tempx;
...
void	draw_julia(t_mlx *mlx)
{
	...
	while (x < WIN_SIZE)
	{
		...
		while (y < WIN_SIZE)
		{
			mlx->f->z_x = (x - WIN_SIZE / 2) / mlx->scale + mlx->center_x;
			mlx->f->z_y = (y - WIN_SIZE / 2) / mlx->scale + mlx->center_y;
			iterations = calculate_julia_plenty(mlx);
